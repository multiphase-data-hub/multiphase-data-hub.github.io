...
本规范不强制统一数据格式，但要求元数据清楚、文件可读、变量含义明确。
...
```text
1. 已处理好的数组数据，例如 .npy、.npz、.h5、.hdf5、.zarr、NetCDF
2. 常见 CFD / 可视化流场文件，例如 .dat、.vtk、.vti、.vtu、.xdmf、EnSight、OpenFOAM 输出
3. 无扩展名或求解器特定二进制文件，例如 Gvol.000001、V.000001、P.000001、VOF.000001
```

前两类属于常见数据格式，只要 `info.json` 说明清楚变量、单位、shape、网格和读取方式即可。
